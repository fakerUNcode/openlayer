...
class MathBase
{
  ...
  static getIdxCoorInLin(coorPnt, coorLin)
  {
    ...
    {
      if (coorLin[i].toString() === coorPnt.toString())
        return i
    }
    return -1
  }
  ...
  static isIntersect(lineCoor1, lineCoor2)
  {
    ...
    var intersect = function(aa, bb, cc, dd)
    {
      ...
      if (mult(cc, dd, aa) * mult(cc, dd, bb) > 0)
        return false
      if (mult(aa, bb, dd) * mult(aa, bb, cc) > 0)
        return false
  ...
  {
    return (pA[0] >= p0[0] && pA[1] >= p0[1] && pB[0] <= p0[0] && pB[1] <= p0[1]) ||
  ...
  {
    if (((pnt[0] - lineCoors[0][0]) * (lineCoors[1][1] - lineCoors[0][1]) === (lineCoors[1][0] - lineCoors[0][0]) * (pnt[1] - lineCoors[0][1])) &&
    (Math.min(lineCoors[0][0], lineCoors[1][0]) <= pnt[0] && pnt[0] <= Math.max(lineCoors[0][0], lineCoors[1][0])) &&
    (Math.min(lineCoors[0][1], lineCoors[1][1]) <= pnt[1] && pnt[1] <= Math.max(lineCoors[0][1], lineCoors[1][1])))
      return true
    else
      return false
  }
  ...
  static pntCrossBrokenLine(linCoors, pntCoor)
  {
    ...
    for (var i = linCoors.length - 1; i > 0; i--)
    {
      ...
      // 取落在线段内，且距离最短的垂足
      if ((pntCross[0] >= linCoors[i][0] && pntCross[0] <= linCoors[i - 1][0]) || (pntCross[0] <= linCoors[i][0] && pntCross[0] >= linCoors[i - 1][0]) ||
        (pntCross[1] >= linCoors[i][1] && pntCross[1] <= linCoors[i - 1][1]) || (pntCross[1] <= linCoors[i][1] && pntCross[1] >= linCoors[i - 1][1]))
      {
  ...
  static radianLine2PI(linCoors)
  {
    ...
    if ((dtaX < 0 && dtaY >= 0) || (dtaX <= 0 && dtaY < 0))
      radian += Math.PI
    else if (dtaX > 0 && dtaY < 0)
      radian += 2 * Math.PI
